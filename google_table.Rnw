...
<<xtable, label=Table:google, echo=FALSE, results=tex>>=
require(xtable)
n <- read.csv('complete.csv', sep=',', header = TRUE)
n<- subset(n, select=c("Round", "GLOSS", "Gaelic", "ParaPart", "google"))
...
xtab<-xtable(n, caption="BLEU scores of Model\\textsubscript{ParaPart} and Google Translation",  label="Table:google", align="llcccc")
names(xtab)<-c('Round', 'Gaelic (Baseline)', 'Gloss', 'ParaPartial', 'Google' )
hlines <- c(-1,0,10, nrow(xtab))
print(xtab, include.rownames =FALSE, hline.after = hlines)
#print(xtab,include.rownames =FALSE, hline.after = hlines, rotate.colnames=TRUE)
@
