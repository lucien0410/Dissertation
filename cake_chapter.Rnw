...
It is not attested that the models trained with gloss data (M=\Sexpr{m_gloss}, SD=\Sexpr{sd_gloss}) outperform the models trained with Gaelic (M=\Sexpr{m_gd}, SD=\Sexpr{sd_gd}); t(9)=\Sexpr{t_val}, p=\Sexpr{p_val}.
