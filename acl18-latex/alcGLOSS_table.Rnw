...
<<xtable, label=Table:GLOSS, echo=FALSE, results=tex>>=
require(xtable)
n <- read.csv('complete.csv', sep=',', header = TRUE)
n<- subset(n, select=c("Round", "Gaelic", "GLOSS"))
gd<-n['Gaelic'][1:10,1]
Treatment<-n['GLOSS'][1:10,1]  # [11,1] is MEAN
...
xtab<-xtable(n, caption="BLEU scores of Model\\textsubscript{GDtoEN} and Model\\textsubscript{GLOSStoEn}",  label="Table:GLOSS", align="llcc")
names(xtab)<-c('Round', 'Gaelic (Baseline)', 'GLOSS' )
