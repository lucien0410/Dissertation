...
First, the samples (i.e. the 3-tuples) are randomly split into three datasets: training set (N=6,388), validation set (N=1,000), and test set (N=1,000). models
...
<<xtable, label=gd_gloss_stat1, echo=FALSE, results=tex>>=
require(xtable)
...
p_val=format(round(r$p.value, 2), nsmall = 2)
xtab2<-xtable(n, caption="BLEU scores of Model\\textsubscript{GLOSStoEN} and Model\\textsubscript{GDtoEN}",  label="Table:BLEUGlossGD")
...
@
The average score of the Models\textsubscript{GLOSStoEN} is only sightly higher than the average score of the Models\textsubscript{GDtoEN}. Also, after doing a paired T-test, the difference between the two types of models is not attested (M\textsubscript{GlossToEn}=\Sexpr{m_gloss}, SD\textsubscript{GlossToEn}=\Sexpr{sd_gloss}, M\textsubscript{GDToEn}=\Sexpr{m_gd}, SD\textsubscript{GDToEn}=\Sexpr{sd_gd}); t(9)=\Sexpr{t_val}, p=\Sexpr{p_val}).
...
In the previous section, we use the \textit{gloss treatment} to attempt to improve the performance. It turns that using gloss line solely is not effective enough to improve the model. However, this result does not falsify the gloss-helps hypothesis; instead, it indicates that combination of the gloss line data and the Gaelic sentence data is necessary. This section reports various ways of combining the gloss line data and the Gaelic sentence data, and the experiments and result using these different treatments. Critically, it is found that when the gloss line data is put parallel to the Gaelic sentence data and each gloss item is put parallel to the corresponding Gaelic word token as the training set, the performance is boosted significantly. The model trained with this special arranged training data also significantly outperforms Google's Gaelic-to-English translation system.    

<<xtable, label=gd_gloss_stat2, echo=FALSE, results=tex>>=
require(xtable)
n <- read.csv('result.tsv.csv', sep='\t', header = TRUE)
#mean(gd)
#mean(gloss)
#sd(gd)
#sd(gloss)
r <-t.test(gd, gloss,paired=TRUE)
xtab2<-xtable(n, caption="More BLEU scores of the Models, where \`ConcatGLOSSGaelic\' is \`GLOSS\\textunderscore Gaelic\', \`GLOSS.Gaelic\' is \`GLOSS Gaelic\', \`hyGD\' is using the less ambigious one from either Gaelic word or gloss with Gaelic word as the default, \`hyGW\' is the same as \`hyGD\' but with the default as gloss.",  label="Table:BLEUGlossGDALL")
#print(xtab2,include.rownames = FALSE, hline.after = c(10))
hlines <- c(-1,0,10, nrow(xtab2))
xtab2<-xtab2
print(xtab2,include.rownames =FALSE, hline.after = hlines)
require(xtable)
n <- read.csv('result2.csv', sep='\t', header = TRUE)
#mean(gd)
#mean(gloss)
#sd(gd)
#sd(gloss)
r <-t.test(gd, gloss,paired=TRUE)
xtab2<-xtable(n, caption="\`Para\' is \`GLOSS -> Gaelic; Gloss -> En; Gaelic -> En\', \`ParaPart\' is \`Para plus Gaelic word token -> Gloss token\', \`Over\' means oversampling, \`Half\' means using half of the training data.",  label="Table:BLEUGlossGDALL")
#print(xtab2,include.rownames = FALSE, hline.after = c(10))
hlines <- c(-1,0,10, nrow(xtab2))
xtab2<-xtab2
print(xtab2,include.rownames =FALSE, hline.after = hlines)
